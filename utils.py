...
# Define YUV color transform
def to_device1(tensor):
    if use_gpu:
        return tensor.to(device='cuda:1')
    else:
        return tensor

def to_device2(tensor):
    if use_gpu:
        return tensor.to(device='cuda:2')
    else:
        return tensor

def to_device3(tensor):
    if use_gpu:
        return tensor.to(device='cuda:3')
    else:
        return tensor


# Corr = np.asarray([[0.26, 0.09, 0.02], [0.27, 0.00, -0.05], [0.27, -0.09, 0.03]]).astype("float32")

# C = torch.from_numpy(Corr)

def aug_canvas(canvas, long_side):
    h = canvas.size(2)
    w = canvas.size(3)

    mx = max(h, w)

    frac = long_side / float(mx)

    h = int(h * frac)
    w = int(w * frac)

    canvas = F.upsample(canvas, (h, w), mode='bilinear').cpu().float()
...
def to_device(tensor):
    if use_gpu:
        return tensor.to(device='cuda:0')
    else:
        return tensor
...
def load_path_for_pytorch(path, max_side=1000, force_scale=False, verbose=True, name=None):
    com_f = max
...
def load_style_folder(scl, phi, paths, scale=-1):
    for p in paths:
        style_im = to_device(
...
def selection_subsampling(out2, paths, regions, ri, samps=100, scale=-1):
    for p in paths:
        X = to_device(
            Variable(load_path_for_pytorch(p, max_side=scale, verbose=False, force_scale=True).unsqueeze(0),
                     requires_grad=False))
        ...
        r = F.upsample(r_temp, (X.size(3), X.size(2)), mode='bilinear')[0, 0, :, :].numpy()
...
def load_style_folder_large(scl, phi, rand, paths, regions, ri, n_samps=-1, subsamps=-1, scale=-1, inner=1,
                            cpu_mode=False):
    if n_samps > 0:
        list.sort(paths)
    ...
    for p in paths:
        ...
        style_im = to_device(
            Variable(load_path_for_pytorch(p, max_side=scale, verbose=False, force_scale=True).unsqueeze(0),
                     requires_grad=False)).half()

        print("style_im : ", style_im.size(), style_im.max(), style_im.min())
        ...
        for j in range(inner):
            ...
            with torch.no_grad():
                zt = phi(scl, style_im, subsamps, r, rand, 'utils')

            if scl in [10]:
                zt = [to_device2(li.view(li.size(0), li.size(1), -1, 1)) for li in zt]
            else:
                zt = [to_device1(li.view(li.size(0), li.size(1), -1, 1)) for li in zt]
